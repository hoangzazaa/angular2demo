...
export class SF0050307Component {
    customerGoalNew: CustomCustomerGoal;
    @Input() index: number;
    ...
    constructor(public sf00503Data: SF00503Data, private router: Router) {
        this.goldCustomerNew[Constants.TYPE_1] = [];
        this.goldCustomerNew[Constants.TYPE_2] = [];
        this.goldCustomerNew[Constants.TYPE_3] = [];
    ...
    updateDataByYear() {
        ...
        if (!this.customerGoalNew.goalItems
            ...
            for (let i = 0; i < 36; i++) {
                this.customerGoalNew.goalItems[i] = new CustomerGoalItem();
                ...
                this.customerGoalNew.goalItems[i].customerType = Constants.CUSTOMER_NEW;
            }
            for (let i = 0; i < 12; i++) {
                ...
                this.goldItemsYear[i].customerType = Constants.CUSTOMER_NEW;
                // push goalItemYear to list goalItems
                this.customerGoalNew.goalItems.push(this.goldItemsYear[i]);
            }
        }

        let goalItems_1 = this.customerGoalNew.goalItems;
                ...
                if (goalItems_1) {
                    goalItems_1.forEach(goalItem => {
                        ...
                        if (i == goalItem.type
                            ...
                            this.goldCustomerNew[i][j] = goalItem.goal;
                        }
                    });
                }
            }
    ...
    calculatorData() {
        ...
        for (let i = 0; i < this.sf00503Data.columns.length; i++) {
            type1New += FormatUtil.isNaN(this.goldCustomerNew[0][i]);
            type2New += FormatUtil.isNaN(this.goldCustomerNew[1][i]);
            type3New += FormatUtil.isNaN(this.goldCustomerNew[2][i]);
            typeYear += FormatUtil.isNaN(this.goldItemsYear[i].goal);
        }

        this.type1New = FormatUtil.isNaN(type1New);
        this.type2New = FormatUtil.isNaN(type2New);
        this.type3New = FormatUtil.isNaN(type3New);
        this.typeYear = FormatUtil.isNaN(typeYear);

        this.totalType = this.type1New + this.type2New + this.type3New;

        for (let i = 0; i < this.sf00503Data.columns.length; i++) {
            this.goldOldCus[i] = FormatUtil.isNaN(this.goldCustomerNew[0][i])
                + FormatUtil.isNaN(this.goldCustomerNew[1][i]) + FormatUtil.isNaN(this.goldCustomerNew[2][i]);
        }
    }
