"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
/**
 * Provider for store header data
 */
var HeaderProvider = (function () {
    function HeaderProvider() {
    }
    HeaderProvider.prototype.reset = function () {
        this.pageName = "";
        this.breadcrumbs = [];
        this._checkHeader = true;
    };
    HeaderProvider.prototype.addBreadCrumb = function (name, router) {
        if (router === void 0) { router = []; }
        var breadcrumb = new BreadCrumbData();
        breadcrumb.name = name;
        breadcrumb.router = router;
        this.breadcrumbs.push(breadcrumb);
    };
    Object.defineProperty(HeaderProvider.prototype, "checkHeader", {
        get: function () {
            return this._checkHeader;
        },
        set: function (value) {
            this._checkHeader = value;
        },
        enumerable: true,
        configurable: true
    });
    HeaderProvider = __decorate([
        core_1.Injectable(), 
        __metadata('design:paramtypes', [])
    ], HeaderProvider);
    return HeaderProvider;
}());
exports.HeaderProvider = HeaderProvider;
var BreadCrumbData = (function () {
    function BreadCrumbData() {
        this.router = [];
    }
    return BreadCrumbData;
}());
exports.BreadCrumbData = BreadCrumbData;
//# sourceMappingURL=Header.provider.js.map