DROP PROCEDURE IF EXISTS `PROC_SFN050602`;

CREATE PROCEDURE `PROC_SFN050602`(IN `departmentId` INT, IN `userId` INT, IN `startDate` VARCHAR(10), IN `endDate` VARCHAR(10), IN `dateType` INT,
                                  IN `method`       INT)
  BEGIN
    SELECT
      i.code              AS code,
      ...
      i.amount            AS amount,
      i.billing_date      AS billingDate,
      i.payment_date      AS dueDate,
      i.method            AS method,
    ...
    FROM
      ...
      LEFT JOIN sfr_sf_payment p ON p.invoice_id = i.id
