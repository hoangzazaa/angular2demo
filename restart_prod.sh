...
log_name="run.log_"`date +%Y%m%d_%H%M%S`
...
# プロセスを立ち上げる
sudo nohup ./"${target_dir}"/bin/sefuri_salesfront -Dhttp.port=80 -Dhttps.port=443 -J-Xms1g -J-Xmx4g -Dthumbnailator.conserveMemoryWorkaround=true > "${log_name}" &
